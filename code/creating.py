...
from managing_file_paths import (
    absolute_path,
    crawl,
    get_filename_and_specific_directory,
    retrieve_code_from,
)


def create_for_each_language(
    language=str, coding_folder_path=str, carbon_copies_path=str
):
    for filepath in crawl(language, coding_folder_path):
        ...
        post_and_save_response(
            retrieve_code_from(filepath),
            filename.replace(" ", "_"),
            specific_directory,
            carbon_copies_path,
        )
...
def post_and_save_response(
    code=str, python_file_name=str, specific_directory=str, carbon_copies_path=str
):
    ...
    r = requests.post(url="https://carbonara-42.herokuapp.com/api/cook", json=data)
